...
if (@ARGV)
{
	...
	if ($VersionSwitch)
	{
		...
		print "\tVer. 20220102 	Specify template & suffix for scf by fltemplate,scfSuffix in lnbts input.\n";
		exit;
	}
...
my $swrel = 'NA';
my $logtypeis ='';
my $numtemplate = -1;	# couting all templates
my %nktemplatedb; 		# saving all templates' info
my %nkuserlabeldb;		# saving all userlabel info
my %nkrmoddb; my %nksmoddb; my %nkswreldb; 	#	saving all rmod/smod/sw's info
for ($inputFileListIndex=0; $inputFileListIndex<=$#inputFileArray; $inputFileListIndex++)
{
	...
	if ($filepath[$#filepath] =~ /(.+).csv$/)  {
		...
		if ($filepath[$#filepath] =~ /template/) {
			...
			$numtemplate ++;
			print "***LOG TYPE IS: $logtypeis ***\n\n";
			$nktemplatedb{$numtemplate}{'filepath'} = $filepath[$#filepath];
			$nktemplatedb{$numtemplate}{'filename'} = $str;
...
#				} elsif ( ($flcel) ) {	
		...
		my ($stext, $sband) = ('','');
		my ($rmodRfm, $rmodRrh) = (FALSE, FALSE);
		foreach my $p ($xpc->findnodes('/ns:raml/ns:cmData/ns:managedObject')) {
			...
			if ((/com.nokia.srbts.eqm:SMOD/) || (/^SMOD$/)) {
				...
				if (/SMOD-(\d+)/) {
					push(@{$nktemplatedb{$numtemplate}{'smodId'}} , $1);
				}
				foreach my $p1 ($xpc->findnodes('./ns:p[@name]',$p)) {
					...
					if (/productName/) {
						if ($p1->nodeType == XML_ELEMENT_NODE) {
							$stext = $p1->findvalue('.');
							if ($stext =~ /(FS\w\w)/) {
								push(@{$nktemplatedb{$numtemplate}{'smodName'}}, $1);
							} else {
								print (">>> Importing template >> SMOD productName $stext not predefined\n");
							}
						}
						last;
					} elsif (/prodCodePlanned/) {
						if ($p1->nodeType == XML_ELEMENT_NODE) {
							$stext = $p1->findvalue('.');
							if ($stext =~ /^([\w\d]+)\.*/) {
								my $scode = $1;
								if (defined $prodCode{$scode}) {
									push (@{$nktemplatedb{$numtemplate}{'smodName'}}, $prodCode{$scode});
								} else {
									print (">>> Importing template >> SMOD proCode $scode not predefined\n");
								}								
							}							
						}
						last;
				...
				$_ = $p->getAttribute("distName");
				if (/RMOD-(\d+)/) {
					push(@{$nktemplatedb{$numtemplate}{'rmodId'}} , $1);
				}				
				foreach my $p1 ($xpc->findnodes('./ns:p[@name]',$p)) {
					$_ = $p1->getAttribute("name");
					if (/productName/) {
						if ($p1->nodeType == XML_ELEMENT_NODE) {
							$stext = $p1->findvalue('.');
							if ($stext =~ /^(\w{4})$/) {
								push(@{$nktemplatedb{$numtemplate}{'rmodName'}}, $1);
							} else {
								print (">>> Importing template >> RMOD productName $stext not predefined\n");
							}							
						}
						last;
					} elsif (/prodCodePlanned/) {
						if ($p1->nodeType == XML_ELEMENT_NODE) {
							...
							if ($stext =~ /^([\w\d]+)\.*/) {
								my $scode = $1;
								if (defined $prodCode{$scode}) {
									push (@{$nktemplatedb{$numtemplate}{'rmodName'}}, $prodCode{$scode});
								} else {
									print (">>> Importing template >> RMOD proCode $scode not predefined\n");
								}								
							}							
						}
						last;
				...
				if (/^([Fx]L\d\d)[_A-Z]/) {
					$swrel = $1;
					$nkswreldb{$nktemplatedb{$numtemplate}{'filename'}} = $swrel;
					#tmp
					#if ($swrel eq 'xL19') {
					#	$swrel = 'FL19';
					#}
		...
		if (($smod > 0) && ($rmod > 0) && ($antl >0) && ($swrel ne 'NA'))  {
			foreach (@{$nktemplatedb{$numtemplate}{'rmodName'}}) {
				my $rmodtype = $_;
				if ($prodType{$rmodtype} eq 'RFM' ) {
					$rmodRfm = TRUE;
				} elsif ($prodType{$rmodtype} eq 'RRH') {
					$rmodRrh = TRUE;
				} else {
					print (">>> Importing template >> RMOD rmodType $rmodtype not predefined\n");
				}
			}
			...
			if ($rmodRrh && ! $rmodRfm) {
				...
				$nkuserlabeldb{$nktemplatedb{$numtemplate}{'filename'}} = $userlabel . ' RRH' . " $sband";
				$nkrmoddb{$nktemplatedb{$numtemplate}{'filename'}} = 'RRH';
				
			} elsif (! $rmodRrh && $rmodRfm) {
				...
				$nkuserlabeldb{$nktemplatedb{$numtemplate}{'filename'}} = $userlabel . ' RFM' . " $sband";				
				$nkrmoddb{$nktemplatedb{$numtemplate}{'filename'}} = 'RFM';
			} elsif ($rmodRrh && $rmodRfm) {
				$fltemplatedb{$swrel}{'MIXED'} =  $inputFileArray[$inputFileListIndex];
				$fltemplatedb{'userlabel'}{'MIXED'}  = 'nS_RFMIXED_BMIXED';
				$nkuserlabeldb{$nktemplatedb{$numtemplate}{'filename'}} = 'nS_RFMIXED_BMIXED';
				$nkrmoddb{$nktemplatedb{$numtemplate}{'filename'}} = 'MIXED';
			}
		} else {
				...
				foreach my $p1 ($xpc->findnodes('./ns:p[@name]',$p)) {
					...
					if (/productName/) {
						if ($p1->nodeType == XML_ELEMENT_NODE) {
							$stext = $p1->findvalue('.');
							if (defined $stext) {
								if ( $prodType{$stext} eq 'RFM') {
									$rmodRfm = TRUE;
								} elsif ($prodType{$stext} eq 'RRH') {
									$rmodRrh = TRUE;
								} else {
									print (">>> Generating template >> product name $stext not predefined\n");
								}
							}
						}
						last;
						...
						if ($p1->nodeType == XML_ELEMENT_NODE) {
							...
							if ($stext =~ /^([\w\d]+)\.*/) {
								my $scode = $1;
								if (defined $prodCode{$scode}) {
									if ( $prodType{$prodCode{$scode}} eq 'RFM') {
										$rmodRfm = TRUE;
									} elsif ($prodType{$prodCode{$scode}} eq 'RRH') {
										$rmodRrh = TRUE;
									}
								} else {
									print (">>> Generating template >> product code $scode not predefined\n");
								}								
							}
						}
						last;
...
if (! $genTemplate) {
	if ($flcel && $flsite && $fltemplate) { 
		foreach (sort keys %sitedb) {
			...
			my $flfilename;	# key for site's label
			if (defined $sitedb{$_}{'fltemplate'}) {
				$siteTemplate = $sitedb{$_}{'fltemplate'};	# input for template file path for site
				my @sfilepath = split(/[\\\/]/,$sitedb{$_}{'fltemplate'});
				if ($sfilepath[$#filepath] =~ /(.+)\.xml$/) {
					$flfilename = $1;
				}
			} else {
				my @sfilepath = split(/[\\\/]/,$siteTemplate);
				if ($sfilepath[$#filepath] =~ /(.+)\.xml$/) {
					$flfilename = $1;
				}
			}
			#print (">>> $swrel, $rmtype \n");
			if ((($rmtype eq 'RRH') || ($rmtype eq 'RFM') || ($rmtype eq 'MIXED')) && (($swrel eq 'FL16') || ($swrel eq 'FL18') || ($swrel eq 'FL19') || ($swrel eq 'xL19') || ($swrel eq 'xL20')) && (defined $siteTemplate)) {
				
				if ($nkswreldb{$flfilename} ne $swrel) {
					print (">>> ! $sitename has diff swRel: $swrel <> $siteTemplate\n");
				} elsif ($nkrmoddb{$flfilename} ne $rmtype) {
					print (">>> ! $sitename has diff rmType: $rmtype <> $siteTemplate\n");
				} else {
						...
						if (/com.nokia.srbts:MRBTS/) {
							...
							foreach my $p1 ($xpc->findnodes('./ns:p[@name]',$p)) {
								...
								if (/userLabel/) {
									if ($p1->nodeType == XML_ELEMENT_NODE) {
										$p1->removeChildNodes();
										if (defined $sitedb{$sitename}{'fltemplate'}) {
											$p1->appendText($nkuserlabeldb{$flfilename});
										} else {
				...
				} elsif (($sitedb{$sitename}{'swrel'} eq "FL19") || ($sitedb{$sitename}{'swrel'} eq "xL19")) {
					# update btsid
					foreach my $p ($xpc->findnodes('/ns:raml/ns:cmData/ns:managedObject')) {
						#btsid in distName attribute obj
						$_ = $p->getAttribute("distName");
				...
				my $out;
				if (defined $sitedb{$sitename}{'scfSuffix'}) {
					open $out, '>', "scf_$sitename$sitedb{$sitename}{'scfSuffix'}.xml";
				} else {
					open $out, '>', "scf_$sitename.xml";
				}
				binmode $out; 
...
{	
	...
	print "*\/*Version 20210801 (v1) *\/\r\n";
	print "*\/*Version 20220102 (v2) - specify template & suffix for scf by fltemplate,scfSuffix in lnbts input *\/\r\n";
